...
def _mi_classif(X, y):
    def _mi_classif_series(x, y):
        ...
        if x_not_na.sum() == 0:
            return 0
        return mutual_info_classif(x[x_not_na].to_frame(), y[x_not_na], random_state=123)[0]
...
def mrmr_selection(X, y, K, relevance = 'mi_class', redundancy = 'spearman', denominator = 'mean'):
    '''
    ...
    Args:
        ...
        relevance:         (str) Relevance method. 
                           string - name of supported methods: 'mi_class' (mutual information for classification), 'mi_regress' (mutual information for regression).
